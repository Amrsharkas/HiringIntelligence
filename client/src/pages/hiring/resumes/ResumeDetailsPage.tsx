...
export default function ResumeDetailsPage() {
  ...
  const DetailedAnalysis = ({ jobScore }: { jobScore: JobScore }) => {
    ...
    // Extract data from new generatedProfile structure if available
    // Support: fullResponse.generatedProfile.brutallyHonestProfile, fullResponse.brutallyHonestProfile, or fullResponse directly
    const generatedProfile = fullResponse?.generatedProfile || fullResponse;
    const brutallyHonestProfile = generatedProfile?.brutallyHonestProfile || fullResponse?.brutallyHonestProfile || generatedProfile;

    // Use brutallyHonestProfile if available (new format), otherwise fall back to fullResponse
    const profileToDisplay = brutallyHonestProfile || fullResponse;

    // Helper function to get a value from profileToDisplay first, then fullResponse as fallback
    const getField = (path: string) => {
      const paths = path.split('.');
      let value = profileToDisplay;
    ...
    // Check if we have detailedBreakdown in the new format or old format
    const detailedBreakdown = profileToDisplay?.detailedBreakdown || fullResponse?.detailedBreakdown;

    if (!fullResponse && !profileToDisplay) {
      return (
    ...
    // If we have the new format (brutallyHonestProfile) but no detailedBreakdown, still show the data
    const hasNewFormat = !!brutallyHonestProfile && !!profileToDisplay?.overallScore;
    const hasOldFormat = !!detailedBreakdown;
    ...
    // Use profileToDisplay as the primary source, with fullResponse as fallback for compatibility
    const dataSource = profileToDisplay || fullResponse;
    ...
    // Helper for overall score color (simple version)
    ...
    const renderNewFormat = () => {
      // Extract honestProfile from fullResponse (it's separate from brutallyHonestProfile)
      const honestProfile = fullResponse?.honestProfile || generatedProfile?.honestProfile;

      return (
        <div className="p-4 space-y-4 bg-white dark:bg-slate-900">
          {/* Overall Score Card (from dataSource or jobScore) */}
          {(dataSource?.overallScore !== undefined || profileToDisplay?.overallScore !== undefined) && (
            <div className="p-4 rounded-lg border-2 border-primary/20 bg-linear-to-r from-primary/10 to-blue-50 dark:from-primary/20 dark:to-blue-900/20">
              <div className="flex items-center justify-between">
                <div>
                  <div className="text-sm font-medium text-slate-600 dark:text-slate-400 mb-1">Overall Match Score</div>
                  <div className={`text-4xl font-bold ${getOverallScoreColor((dataSource?.overallScore ?? profileToDisplay?.overallScore) || 0)}`}>
                    {(dataSource?.overallScore ?? profileToDisplay?.overallScore) || 0}%
                  </div>
                </div>
          ...
          {/* Honest Profile - Critical Assessment */}
          {honestProfile && (
            <div className="p-4 rounded-lg border-2 border-orange-200 dark:border-orange-800 bg-linear-to-r from-orange-50 to-amber-50 dark:from-orange-900/20 dark:to-amber-900/20">
              <h5 className="font-bold text-lg mb-4 flex items-center gap-2 text-orange-900 dark:text-orange-300">
                <FileText className="h-5 w-5" />
                Honest Assessment
              </h5>

              {/* Profile Summary */}
              {honestProfile.profileSummary && (
                <div className="mb-4 p-3 bg-white dark:bg-slate-800 rounded-lg border border-orange-200 dark:border-orange-700">
                  <div className="text-sm font-semibold text-orange-800 dark:text-orange-300 mb-2">Summary</div>
                  <p className="text-sm text-slate-700 dark:text-slate-300">{honestProfile.profileSummary}</p>
                </div>
              )}

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                {/* Strengths */}
                {honestProfile.strengths && honestProfile.strengths.length > 0 && (
                  <div className="p-3 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-700">
                    <div className="text-sm font-semibold text-green-800 dark:text-green-300 mb-2 flex items-center gap-2">
                      <CheckCircle className="h-4 w-4" />
                      Strengths ({honestProfile.strengths.length})
                    </div>
                    <ul className="space-y-1">
                      {honestProfile.strengths.map((strength: string, i: number) => (
                ...
                {/* Critical Weaknesses */}
                {honestProfile.criticalWeaknesses && honestProfile.criticalWeaknesses.length > 0 && (
                  <div className="p-3 bg-red-50 dark:bg-red-900/20 rounded-lg border border-red-200 dark:border-red-700">
                    <div className="text-sm font-semibold text-red-800 dark:text-red-300 mb-2 flex items-center gap-2">
                      <AlertTriangle className="h-4 w-4" />
                      Critical Weaknesses ({honestProfile.criticalWeaknesses.length})
                    </div>
                    <ul className="space-y-1">
                      {honestProfile.criticalWeaknesses.map((weakness: string, i: number) => (
              ...
              {/* Skill Assessment & Experience Verification */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                {honestProfile.skillAssessment && (
                  <div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-700">
                    <div className="text-sm font-semibold text-blue-800 dark:text-blue-300 mb-2">Skill Assessment</div>
                    <p className="text-sm text-slate-700 dark:text-slate-300">{honestProfile.skillAssessment}</p>
                  </div>
                )}

                {honestProfile.experienceVerification && (
                  <div className="p-3 bg-purple-50 dark:bg-purple-900/20 rounded-lg border border-purple-200 dark:border-purple-700">
                    <div className="text-sm font-semibold text-purple-800 dark:text-purple-300 mb-2">Experience Verification</div>
                    <p className="text-sm text-slate-700 dark:text-slate-300">{honestProfile.experienceVerification}</p>
                  </div>
                )}
              </div>

              {/* Red Flags */}
              {honestProfile.redFlags && honestProfile.redFlags.length > 0 && (
                <div className="mb-4 p-3 bg-red-50 dark:bg-red-900/20 rounded-lg border border-red-200 dark:border-red-700">
                  <div className="text-sm font-semibold text-red-800 dark:text-red-300 mb-2 flex items-center gap-2">
                    <AlertTriangle className="h-4 w-4" />
                    Red Flags ({honestProfile.redFlags.length})
                  </div>
                  <ul className="space-y-1">
                    {honestProfile.redFlags.map((flag: string, i: number) => (
                      <li key={i} className="text-sm text-red-700 dark:text-red-300 flex items-start gap-2">
                        <span className="text-red-600 mt-1">✗</span>
                        <span>{flag}</span>
                      </li>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                {honestProfile.hirabilityScore !== undefined && (
                  <div className="p-3 bg-indigo-50 dark:bg-indigo-900/20 rounded-lg border border-indigo-200 dark:border-indigo-700 text-center">
                    <div className="text-xs font-semibold text-indigo-800 dark:text-indigo-300 mb-1">Hirability Score</div>
                    <div className={`text-3xl font-bold ${getOverallScoreColor(honestProfile.hirabilityScore * 10)}`}>
                      {honestProfile.hirabilityScore}/10
                    </div>
                  </div>
                )}

                {honestProfile.recommendedRole && (
                  <div className="p-3 bg-teal-50 dark:bg-teal-900/20 rounded-lg border border-teal-200 dark:border-teal-700">
                    <div className="text-xs font-semibold text-teal-800 dark:text-teal-300 mb-1">Recommended Role</div>
                    <div className="text-sm font-medium text-slate-700 dark:text-slate-300">{honestProfile.recommendedRole}</div>
                  </div>
                )}
                ...
                {honestProfile.salaryRange && (
                  <div className="p-3 bg-emerald-50 dark:bg-emerald-900/20 rounded-lg border border-emerald-200 dark:border-emerald-700">
                    <div className="text-xs font-semibold text-emerald-800 dark:text-emerald-300 mb-1">Salary Range</div>
                    <div className="text-sm font-medium text-slate-700 dark:text-slate-300">{honestProfile.salaryRange}</div>
                  </div>
                )}
              </div>
              ...
              {honestProfile.notes && (
                <div className="p-3 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg border border-yellow-200 dark:border-yellow-700">
                  <div className="text-sm font-semibold text-yellow-800 dark:text-yellow-300 mb-2">Additional Notes</div>
                  <p className="text-sm text-slate-700 dark:text-slate-300">{honestProfile.notes}</p>
                </div>
              )}
            </div>
          )}
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  {fullResponse.executiveSummary.fitScore && (
                    <Badge className={`text-xs ${fullResponse.executiveSummary.fitScore === 'EXCELLENT' ? 'bg-green-500' :
                  ...
                  {fullResponse.executiveSummary.hiringUrgency && (
                    <Badge className={`text-xs ${fullResponse.executiveSummary.hiringUrgency === 'EXPEDITE' ? 'bg-green-600' :
              ...
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  {fullResponse.verdict.confidence && (
                    <span className={`text-xs font-medium px-2 py-1 rounded ${fullResponse.verdict.confidence === 'HIGH' ? 'bg-green-100 text-green-700' :
                  ...
                  {fullResponse.verdict.riskLevel && (
                    <span className={`text-xs font-medium px-2 py-1 rounded ${fullResponse.verdict.riskLevel === 'LOW' ? 'bg-green-100 text-green-700' :
          ...
          {(() => {
            const sectionA = dataSource?.sectionA ?? profileToDisplay?.sectionA ?? fullResponse?.sectionA;
            const sectionB = dataSource?.sectionB ?? profileToDisplay?.sectionB ?? fullResponse?.sectionB;
            const sectionC = dataSource?.sectionC ?? profileToDisplay?.sectionC ?? fullResponse?.sectionC;
            const sectionD = dataSource?.sectionD ?? profileToDisplay?.sectionD ?? fullResponse?.sectionD;
            const sectionE = dataSource?.sectionE ?? profileToDisplay?.sectionE ?? fullResponse?.sectionE;
            const sectionF = dataSource?.sectionF ?? profileToDisplay?.sectionF ?? fullResponse?.sectionF;
            const sectionABreakdown = detailedBreakdown?.sectionA;

            if (sectionA === undefined && sectionB === undefined) return null;
                    ...
                    <AccordionItem value="sectionA" className="bg-white dark:bg-slate-800 rounded border dark:border-gray-600 overflow-hidden">
                      <AccordionTrigger className="px-3 py-2 hover:no-underline hover:bg-gray-50 dark:hover:bg-slate-700">
                        <div className="flex items-center justify-between w-full pr-2">
                          ...
                          <div className="flex items-center gap-2">
                            <span className="text-lg font-bold text-blue-700">{sectionA ?? '-'}<span className="text-xs font-normal text-gray-500 dark:text-gray-400">/30</span></span>
                            <span className="text-xs text-gray-500 dark:text-gray-400">({sectionA !== undefined ? Math.round((sectionA / 30) * 100) : 0}%)</span>
                          </div>
                        </div>
                      </AccordionTrigger>
                      <AccordionContent className="px-3 pb-3">
                        <div className="space-y-2 pt-2 border-t border-gray-200 dark:border-gray-600">
                          {sectionABreakdown.A1_technicalKnowledge && renderSubsectionItem('A1: Technical Knowledge', sectionABreakdown.A1_technicalKnowledge, 15)}
                          {sectionABreakdown.A1_skillsMatch && renderSubsectionItem('A1: Skills Match', sectionABreakdown.A1_skillsMatch, 15)}
                          {sectionABreakdown.A1_coreTechStackMatch && renderSubsectionItem('A1: Core Skills Match', sectionABreakdown.A1_coreTechStackMatch, 15)}
                          {sectionABreakdown.A2_problemSolving && renderSubsectionItem('A2: Problem Solving', sectionABreakdown.A2_problemSolving, 10)}
                          {sectionABreakdown.A2_skillDepth && renderSubsectionItem('A2: Skill Depth & Recency', sectionABreakdown.A2_skillDepth, 10)}
                          {sectionABreakdown.A2_skillRecency && renderSubsectionItem('A2: Skill Recency', sectionABreakdown.A2_skillRecency, 10)}
                          {sectionABreakdown.A3_practicalApplication && renderSubsectionItem('A3: Practical Application', sectionABreakdown.A3_practicalApplication, 5)}
                          {sectionABreakdown.A3_toolsMatch && renderSubsectionItem('A3: Tools/Systems Match', sectionABreakdown.A3_toolsMatch, 5)}
                          {sectionABreakdown.A3_requiredToolVolume && renderSubsectionItem('A3: Required Tools', sectionABreakdown.A3_requiredToolVolume, 5)}
                        </div>
                      </AccordionContent>
                    </AccordionItem>
                  )}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div className="p-3 bg-green-50 dark:bg-green-900/30 rounded-lg border border-green-200 dark:border-green-700 text-center">
              <div className="text-2xl font-bold text-green-700">
                {dataSource?.strengthsHighlights?.length || fullResponse.strengthsHighlights?.length || 0}
              </div>
            ...
            <div className="p-3 bg-red-50 dark:bg-red-900/30 rounded-lg border border-red-200 dark:border-red-700 text-center">
              <div className="text-2xl font-bold text-red-700">
                {dataSource?.improvementAreas?.length || fullResponse.improvementAreas?.length || 0}
              </div>
            ...
            <div className="p-3 bg-amber-50 dark:bg-amber-900/30 rounded-lg border border-amber-200 dark:border-amber-700 text-center">
              <div className="text-2xl font-bold text-amber-700">
                {dataSource?.skillAnalysis?.matchedSkills?.length || fullResponse.skillAnalysis?.matchedSkills?.length || 0}
              </div>
            ...
            <div className="p-3 bg-purple-50 dark:bg-purple-900/30 rounded-lg border border-purple-200 dark:border-purple-700 text-center">
              <div className="text-2xl font-bold text-purple-700">
                {dataSource?.skillAnalysis?.missingSkills?.length || fullResponse.skillAnalysis?.missingSkills?.length || 0}
              </div>
              <div className="text-xs text-purple-600">Skills Missing</div>
            </div>
          </div>

          {/* Additional Score Cards: Answer Quality, CV Consistency, Gap Severity */}
          {(() => {
            const answerQualityScore = dataSource?.answerQualityScore ?? fullResponse?.answerQualityScore;
            const cvConsistencyScore = dataSource?.cvConsistencyScore ?? fullResponse?.cvConsistencyScore;
            const gapSeverityScore = dataSource?.gapSeverityScore ?? fullResponse?.gapSeverityScore;

            if (answerQualityScore === undefined && cvConsistencyScore === undefined && gapSeverityScore === undefined) return null;
            ...
            const getScoreBgColorLocal = (score: number) => {
            ...
            return (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                {answerQualityScore !== undefined && (
                  <div className={`p-3 rounded-lg border text-center ${getScoreBgColorLocal(answerQualityScore)}`}>
                    <div className={`text-2xl font-bold ${getScoreColorLocal(answerQualityScore)}`}>
                      {answerQualityScore}
                    </div>
                    <div className="text-xs text-gray-600 dark:text-gray-400 mt-1">Answer Quality Score</div>
                    <div className="text-xs text-gray-500 dark:text-gray-500 mt-1">/100</div>
                  </div>
                )}
                {cvConsistencyScore !== undefined && (
                  <div className={`p-3 rounded-lg border text-center ${getScoreBgColorLocal(cvConsistencyScore)}`}>
                    <div className={`text-2xl font-bold ${getScoreColorLocal(cvConsistencyScore)}`}>
                      {cvConsistencyScore}
                    </div>
                    <div className="text-xs text-gray-600 dark:text-gray-400 mt-1">CV Consistency Score</div>
                    <div className="text-xs text-gray-500 dark:text-gray-500 mt-1">/100</div>
                  </div>
                )}
                {gapSeverityScore !== undefined && (
                  <div className={`p-3 rounded-lg border text-center ${getScoreBgColorLocal(gapSeverityScore)}`}>
                    <div className={`text-2xl font-bold ${getScoreColorLocal(gapSeverityScore)}`}>
                      {gapSeverityScore}
                    </div>
                    <div className="text-xs text-gray-600 dark:text-gray-400 mt-1">Gap Severity Score</div>
                    <div className="text-xs text-gray-500 dark:text-gray-500 mt-1">/100</div>
                  </div>
                )}
              </div>
                ...
                <div className="mt-2 p-2 bg-white/60 dark:bg-slate-800/60 rounded border border-indigo-100 dark:border-indigo-800">
                  <div className="flex items-center gap-2 mb-1">
                    {fullResponse.domainAnalysis.domainRiskLevel && (
                      <Badge variant="outline" className={`text-xs ${fullResponse.domainAnalysis.domainRiskLevel === 'CRITICAL' ? 'bg-red-100 text-red-700 border-red-300' :
                      ...
                      typeof gap === 'string' ? (
                        ...
                        <div key={i} className="p-2 bg-red-50/50 rounded border border-red-200">
                          <div className="flex items-center justify-between">
                            ...
                            {gap.importance && (
                              <Badge variant="outline" className={`text-xs ${gap.importance === 'CRITICAL' ? 'bg-red-100 text-red-700 border-red-300' :
              ...
              {fullResponse.domainAnalysis.domainHiringRecommendation && (
                <div className="mt-3 p-3 bg-white/80 dark:bg-slate-800/80 rounded border border-indigo-200 dark:border-indigo-700">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex items-center gap-2">
                      <Badge className={`text-xs ${fullResponse.domainAnalysis.domainHiringRecommendation === 'PROCEED' ? 'bg-green-100 text-green-700 border-green-300' :
                      ...
                      {fullResponse.domainAnalysis.domainTransitionSuccess && (
                        <Badge variant="outline" className={`text-xs ${fullResponse.domainAnalysis.domainTransitionSuccess === 'HIGH' ? 'bg-green-50 text-green-700 border-green-300' :
              ...
              {fullResponse.experienceAnalysis.seniorityMatch && (
                <div className="p-2 bg-white dark:bg-slate-800 rounded border dark:border-gray-600 mb-2">
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <Badge variant="outline" className={`text-xs ${fullResponse.experienceAnalysis.seniorityMatch.match === 'EXACT' ? 'bg-green-50 text-green-700' :
                ...
                <div className="mb-2">
                  ...
                  <div className="flex flex-wrap gap-1 mt-1">
                    {fullResponse.experienceAnalysis.industryExperience.map((ind: any, i: number) => (
                      <Badge key={i} variant="outline" className={`text-xs ${ind.relevance === 'HIGH' ? 'bg-green-50 text-green-700 border-green-200' :
                ...
                <div className="mb-2">
                  ...
                  <div className="space-y-1 mt-1">
                    {fullResponse.experienceAnalysis.employmentGaps.map((gap: any, i: number) => (
                      <div key={i} className={`text-xs p-2 bg-white rounded border ${gap.severity === 'SIGNIFICANT' ? 'border-red-200' :
                        gap.severity === 'MODERATE' ? 'border-orange-200' :
                          'border-yellow-200'
                        }`}>
                        <div className="flex items-center justify-between">
                          <span>{gap.gapStart} - {gap.gapEnd}</span>
                          <Badge variant="outline" className={`text-xs ${gap.severity === 'SIGNIFICANT' ? 'bg-red-50 text-red-700' :
                      ...
                      <div key={i} className="text-xs p-3 bg-white dark:bg-slate-800 rounded border dark:border-gray-600">
                        ...
                        <div className="flex items-start justify-between mb-2">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {role.relevance && (
                              <Badge variant="outline" className={`text-xs ${role.relevance === 'HIGH' ? 'bg-green-50 text-green-700' :
                        ...
                        <div className="flex items-center gap-3 text-gray-500 mb-2">
                          ...
                          {role.promotionIndicator && role.promotionIndicator !== 'UNKNOWN' && (
                            <Badge variant="outline" className={`text-xs ${role.promotionIndicator === 'PROMOTED' ? 'bg-green-50 text-green-700 border-green-200' :
                ...
                <div className="mt-2 p-2 bg-white/60 rounded border border-blue-100">
                  ...
                  {fullResponse.experienceAnalysis.tenureAnalysis.pattern && (
                    <div className="flex items-center gap-2 mt-2">
                      <Badge variant="outline" className={`text-xs ${fullResponse.experienceAnalysis.tenureAnalysis.pattern === 'STABLE' ? 'bg-green-50 text-green-700' :
          ...
          {(dataSource?.interviewRecommendations || fullResponse.interviewRecommendations) && (
            <div className="p-3 bg-linear-to-r from-teal-50 to-cyan-50 dark:from-teal-900/30 dark:to-cyan-900/30 rounded-lg border border-teal-200 dark:border-teal-700">
              ...
              {(() => {
                const interviewRecs = dataSource?.interviewRecommendations || fullResponse.interviewRecommendations;
                return typeof interviewRecs === "object" && !Array.isArray(interviewRecs) ? (
                  <div className="space-y-3">
                    {interviewRecs.mustExplore?.length > 0 && (
                      <div>
                        <div className="text-xs font-semibold text-teal-700 dark:text-teal-300 mb-1">Must Explore</div>
                        <ul className="space-y-1">
                          {interviewRecs.mustExplore.map((item: string, i: number) => (
                    ...
                    {interviewRecs.technicalValidation?.length > 0 && (
                      <div>
                        <div className="text-xs font-semibold text-blue-700 mb-1">Technical Validation</div>
                        <ul className="space-y-1">
                          {interviewRecs.technicalValidation.map((item: string, i: number) => (
                            <li key={i} className="text-sm flex items-start gap-2">
                              <span className="text-primary">✓</span>
                              <span>{item}</span>
                            </li>
                ...
                ) : Array.isArray(interviewRecs) ? (
                  <ul className="space-y-1">
                    {interviewRecs.map((rec: string, i: number) => (
                      <li key={i} className="text-sm flex items-start gap-2">
          ...
          {(() => {
            const followUpQuestions = dataSource?.followUpQuestions || fullResponse?.followUpQuestions;
            if (!followUpQuestions) return null;

            const hasQuestions = (followUpQuestions.technical && followUpQuestions.technical.length > 0) ||
              (followUpQuestions.behavioral && followUpQuestions.behavioral.length > 0) ||
              (followUpQuestions.leadership && followUpQuestions.leadership.length > 0) ||
              (followUpQuestions.cultural && followUpQuestions.cultural.length > 0) ||
              (followUpQuestions.clarification && followUpQuestions.clarification.length > 0);

            if (!hasQuestions) return null;

            return (
              <div className="p-3 bg-linear-to-r from-blue-50 to-indigo-50 dark:from-blue-900/30 dark:to-indigo-900/30 rounded-lg border border-blue-200 dark:border-blue-700">
                <h5 className="font-medium text-sm mb-3 flex items-center gap-2 text-blue-800 dark:text-blue-300">
                  <FileQuestion className="h-4 w-4" />
                  Follow-up Questions for Final Interview
                </h5>
                <div className="space-y-3">
                  {followUpQuestions.technical && followUpQuestions.technical.length > 0 && (
                    <div>
                      <div className="text-xs font-semibold text-blue-700 dark:text-blue-300 mb-1">Technical Questions</div>
                      <ul className="space-y-1">
                        {followUpQuestions.technical.map((q: string, i: number) => (
                          <li key={i} className="text-sm flex items-start gap-2">
                  ...
                  {followUpQuestions.behavioral && followUpQuestions.behavioral.length > 0 && (
                    <div>
                      ...
                      <ul className="space-y-1">
                        {followUpQuestions.behavioral.map((q: string, i: number) => (
                          <li key={i} className="text-sm flex items-start gap-2">
                  ...
                  {followUpQuestions.leadership && followUpQuestions.leadership.length > 0 && (
                    <div>
                      ...
                      <ul className="space-y-1">
                        {followUpQuestions.leadership.map((q: string, i: number) => (
                          <li key={i} className="text-sm flex items-start gap-2">
                  ...
                  {followUpQuestions.cultural && followUpQuestions.cultural.length > 0 && (
                    <div>
                      ...
                      <ul className="space-y-1">
                        {followUpQuestions.cultural.map((q: string, i: number) => (
                          <li key={i} className="text-sm flex items-start gap-2">
                            <span className="text-teal-600">•</span>
                            <span>{q}</span>
                          </li>
                  ...
                  {followUpQuestions.clarification && followUpQuestions.clarification.length > 0 && (
                    <div>
                      ...
                      <ul className="space-y-1">
                        {followUpQuestions.clarification.map((q: string, i: number) => (
                          <li key={i} className="text-sm flex items-start gap-2">
          ...
          {(() => {
            const hiringRec = dataSource?.hiringRecommendation || fullResponse?.hiringRecommendation;
            if (!hiringRec || (!hiringRec.nextSteps && !hiringRec.onboardingRecommendations)) return null;

            return (
              <div className="p-3 bg-linear-to-r from-emerald-50 to-green-50 dark:from-emerald-900/30 dark:to-green-900/30 rounded-lg border border-emerald-200 dark:border-emerald-700">
                <h5 className="font-medium text-sm mb-3 flex items-center gap-2 text-emerald-800 dark:text-emerald-300">
                  <Target className="h-4 w-4" />
                  Recommended Next Steps
                </h5>
                <div className="space-y-3">
                  {hiringRec.nextSteps && hiringRec.nextSteps.length > 0 && (
                    <div>
                      <div className="text-xs font-semibold text-emerald-700 dark:text-emerald-300 mb-1">Next Steps</div>
                      <ul className="space-y-1">
                        {hiringRec.nextSteps.map((step: string, i: number) => (
                          <li key={i} className="text-sm flex items-start gap-2">
                            <span className="text-emerald-600">→</span>
                            <span>{step}</span>
                          </li>
                  ...
                  {hiringRec.onboardingRecommendations && (
                    <div className="space-y-2">
                      {hiringRec.onboardingRecommendations.firstWeekPriorities && hiringRec.onboardingRecommendations.firstWeekPriorities.length > 0 && (
                        <div>
                          <div className="text-xs font-semibold text-green-700 dark:text-green-300 mb-1">First Week Priorities</div>
                          <ul className="space-y-1">
                            {hiringRec.onboardingRecommendations.firstWeekPriorities.map((item: string, i: number) => (
                              <li key={i} className="text-sm flex items-start gap-2">
                      ...
                      {hiringRec.onboardingRecommendations.firstMonthGoals && hiringRec.onboardingRecommendations.firstMonthGoals.length > 0 && (
                        <div>
                          <div className="text-xs font-semibold text-blue-700 dark:text-blue-300 mb-1">First Month Goals</div>
                          <ul className="space-y-1">
                            {hiringRec.onboardingRecommendations.firstMonthGoals.map((item: string, i: number) => (
                              <li key={i} className="text-sm flex items-start gap-2">
                                <span className="text-blue-600">•</span>
                                <span>{item}</span>
                              </li>
                      ...
                      {hiringRec.onboardingRecommendations.trainingNeeds && hiringRec.onboardingRecommendations.trainingNeeds.length > 0 && (
                        <div>
                          <div className="text-xs font-semibold text-amber-700 dark:text-amber-300 mb-1">Training Needs</div>
                          <ul className="space-y-1">
                            {hiringRec.onboardingRecommendations.trainingNeeds.map((item: string, i: number) => (
                              <li key={i} className="text-sm flex items-start gap-2">
                                <span className="text-amber-600">•</span>
                                <span>{item}</span>
                              </li>
                      ...
                      {hiringRec.onboardingRecommendations.mentorshipNeeds && (
                        <div>
                          <div className="text-xs font-semibold text-purple-700 dark:text-purple-300 mb-1">Mentorship Needs</div>
                          <div className="text-sm">{hiringRec.onboardingRecommendations.mentorshipNeeds}</div>
                        </div>
                      )}
                    </div>
                  )}
          ...
          {/* Psycholinguistic Analysis - Contradictions */}
          {(() => {
            const psychAnalysis = dataSource?.psycholinguisticAnalysis || fullResponse?.psycholinguisticAnalysis;
            const contradictions = psychAnalysis?.authenticity?.contradictions;

            if (!contradictions || contradictions.length === 0) return null;

            return (
              <div className="p-3 bg-linear-to-r from-orange-50 to-amber-50 dark:from-orange-900/30 dark:to-amber-900/30 rounded-lg border border-orange-200 dark:border-orange-700">
                <h5 className="font-medium text-sm mb-3 flex items-center gap-2 text-orange-800 dark:text-orange-300">
                  <AlertTriangle className="h-4 w-4" />
                  Contradictions from CV
                </h5>
                <div className="space-y-2">
                  {contradictions.map((contradiction: string, i: number) => (
                    <div key={i} className="p-2 bg-white dark:bg-slate-800 rounded border border-orange-200 dark:border-orange-700">
          ...
          {/* Red Flags */}
          {(() => {
            const redFlags = dataSource?.redFlags || fullResponse?.redFlags;
